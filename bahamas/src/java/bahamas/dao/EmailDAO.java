...
public class EmailDAO {
    ...
    public static ArrayList<Email> retrieveAllUnverifiedEmail(int cid) {
        ArrayList<Email> emailList = new ArrayList<Email>();
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        ...
        try {
            conn = ConnectionManager.getConnection();
            stmt = conn.prepareStatement("SELECT CONTACT_ID, DATE_CREATED, CREATED_BY, EMAIL, REMARKS, DATE_OBSOLETE, VERIFIED FROM EMAIL WHERE VERIFIED = 0 AND CONTACT_ID = ?");
            stmt.setInt(1, cid);
            rs = stmt.executeQuery();
            while (rs.next()) {
                int contactId = rs.getInt(1);
