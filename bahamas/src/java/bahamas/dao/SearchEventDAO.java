...
public class SearchEventDAO {
    ...
    public static HashMap<Integer, Event> searchEventByTitleLocationDate(String eventTitleValue, String locationValue, Date startDate, Date endDate, boolean isOtherLocation) {
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        boolean isFirst = true;
        HashMap<Integer, Event> eventHM = new HashMap<Integer, Event>();
        String prepareStatement = "SELECT CREATED_BY, DATE_CREATED, EVENT_TITLE, "
        ...
        try {
            ...
            if(eventTitleValue != null){
                prepareStatement += " WHERE EVENT_TITLE LIKE ? ";
                isFirst = false;
            }
            if (locationValue != null && !isOtherLocation) {
                if(!isFirst){
                    prepareStatement += "AND EVENT_LOCATION_NAME LIKE ? ";
                } else {
                    prepareStatement += " WHERE EVENT_LOCATION_NAME LIKE ? ";
                    isFirst = false;
                }
            } else if (locationValue != null && isOtherLocation){
                if(!isFirst){
                    prepareStatement += "AND ADDRESS LIKE ? ";
                } else {
                    prepareStatement += " WHERE ADDRESS LIKE ? ";
                    isFirst = false;
                }
            }
            if (startDate != null) {
                if(!isFirst){
                    prepareStatement += "AND EVENT_START_DATE >= ? ";
                } else {
                    prepareStatement += " WHERE EVENT_START_DATE >= ? ";
                    isFirst = false;
                }
            }
            if (endDate != null) {
            ...
            int count = 1;
            if(eventTitleValue != null){
                stmt.setString(count++, "%" + eventTitleValue + "%");
            }
            if (locationValue != null) {
                stmt.setString(count++, locationValue);
            }
            if (startDate != null) {
                stmt.setDate(count++, new java.sql.Date(startDate.getTime()));
            }
            if (endDate != null) {
                stmt.setDate(count++, new java.sql.Date(endDate.getTime()));
            }
