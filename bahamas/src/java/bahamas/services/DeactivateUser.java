 ...
 * @author tan.si.hao
 */
@WebServlet(name = "DeactivateUser", urlPatterns = {"/user.deactivate"})
            ...
            if (jsonLine == null || jsonLine.isEmpty()) {
                ...
                if (contact != null && contact.isIsAdmin()) {
                    ...
                    for (int i = 0; i < contactIdJsonArray.size() - 1; i++) {
                        ...
                        contactIdStr += contactIdTemp + ", ";
                    }
                    contactIdStr += contactIdJsonArray.get(contactIdJsonArray.size() - 1).getAsString();
                    if (contactDAO.updateToDeactive(contactIdJsonArray)) {
                        AuditLogDAO.insertAuditLog(username, "Deactivate User", "Deactivate user: Contact ID: " + contactIdStr);
                    } else {
                        json.addProperty("message", "Fail to update status");
                        out.println(gson.toJson(json));
                        return;
                    }
